                                         ...
                                         dci_dl_rnti_config_type                      dci_type) const
{
  ...
  //       if (logfile.is_open()) {
  //           logfile << "Raw PRBs needed: " << static_cast<unsigned int>(nof_prbs);  
  //           logfile.close();
  //       }
...
int ue_cell::handle_crc_pdu(slot_point pusch_slot, const ul_crc_pdu_indication& crc_pdu)
{
  ...
  if (tbs >= 0) {
    ...
    if (crc_pdu.ul_sinr_dB.has_value()) {
      // float snr_value = crc_pdu.ul_sinr_dB.value();
      channel_state.update_pusch_snr(crc_pdu.ul_sinr_dB.value());

      // // Log the SNR
      // std::ofstream logfile("channel-log.txt", std::ios_base::app); // Open log file in append mode
      // if (logfile.is_open()) {
      //     logfile << "TTI: " << edgeric::tti_cnt << ", RNTI: " << static_cast<unsigned int>(crnti_)  << ", SNR: " << snr_value << std::endl;
      //     logfile.close();
      // } 
    }
  }
...
void ue_cell::handle_csi_report(const csi_report_data& csi_report)
{
  ...
  //   unsigned int cqi_value = csi_report.first_tb_wideband_cqi.value().to_uint();
  //   std::ofstream logfile("channel-log.txt", std::ios_base::app); // Open log file in append mode
  //   if (logfile.is_open()) {
  //       logfile <<  "TTI: " << edgeric::tti_cnt << ", RNTI: " << static_cast<unsigned int>(ue_cfg->crnti) << " CQI: " << cqi_value << std::endl;
  //       logfile.close();
  //   } else {
  //       logger.error("Failed to open CQI log file.");
  //   }
  // }
