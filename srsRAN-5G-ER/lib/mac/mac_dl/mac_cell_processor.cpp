    ...
    for (const dl_msg_alloc& grant : dl_res.ue_grants) {
      for (const dl_msg_tb_info& tb_info : grant.tb_list) {
        for (const dl_msg_lc_info& lc_info : tb_info.lc_chs_to_sched) {
          ...
          // Log the DL buffer state and RNTI
          std::ofstream logFile("scheduling-final.txt", std::ios::app); // Open log.txt for appending
          logFile << "DL Buffer State: " << bs.bs << ", RNTI: " << static_cast<unsigned int>(grant.pdsch_cfg.rnti) << std::endl;
          logFile.close();
