...
def collect_samples(pid, queue, env, policy, custom_reward,
                    mean_action, render, running_state, min_batch_size, messenger):
    if pid > 0:
    ...
    while num_steps < min_batch_size:
        ...
        for t in range(10000):  # Adjust the range as needed
            ...
            # Get metrics using the messenger
            ran_index, ue_data = messenger.get_metrics(True)
            numues = len(ue_data)
            ...
            # Send scheduling weight using the messenger
            messenger.send_scheduling_weight(ran_index, weight, True)
    ...
    def collect_samples(self, min_batch_size, mean_action=False, render=False):
        ...
        for i in range(self.num_threads - 1):
            worker_args = (i + 1, queue, self.env, self.policy, self.custom_reward, mean_action,
                           False, self.running_state, thread_batch_size, self.messenger)
            workers.append(multiprocessing.Process(target=collect_samples, args=worker_args))
        for worker in workers:
            worker.start()

        memory, log = collect_samples(0, None, self.env, self.policy, self.custom_reward, mean_action,
                                      render, self.running_state, thread_batch_size, self.messenger)
