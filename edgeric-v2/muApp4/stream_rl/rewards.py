...
@register_reward("stall_based")
def stall_based_reward(total_bytes_transferred, backlog_lens, stall_statuses=None):
    reward = 0
    for stalling in stall_statuses:
        if stalling:
            reward -= 10
        else:
            reward += 2
...
def throughput(total_bytes_transferred, backlog_lens, stall_statuses=None):
    return total_bytes_transferred


@register_reward("negative_backlog_len")
def neg_bl(total_bytes_transferred, backlog_lens, stall_statuses=None):
