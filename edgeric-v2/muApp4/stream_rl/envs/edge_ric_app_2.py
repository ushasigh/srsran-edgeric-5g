...
@register_env("EdgeRICApp2")
class EdgeRICApp2(gym.Env):
    """EdgeRIC Env: Simulation of the realtime RIC setup + Application (Model 2)"""
    ...
    def __init__(self, config: EnvContext):
        ...
        self.request_threshold = config["request_threshold"]
    ...
    def step(self, action):
        ...
        for ue in range(self.num_UEs):
            ...
            self.media_caches[ue] += float(bytes_transferred) / chunk_size
            completed_chunks = np.floor(self.media_caches[ue])
            self.media_caches[ue] -= completed_chunks
            self.media_lens[ue] += completed_chunks

            if self.t % inter_departure_time == 0:
                if self.media_lens[ue] > 0:
                    self.stalling[ue] = False
                    self.media_lens[ue] -= 1
                else:
                    self.stalling[ue] = True
                    self.stall_counts[ue] += 1

            # Update BLs
            _, chunk_size = self.playout_process_params[ue]
            if self.media_lens[ue] < self.request_threshold:
