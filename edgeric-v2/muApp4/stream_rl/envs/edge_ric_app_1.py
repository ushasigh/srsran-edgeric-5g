...
@register_env("EdgeRICApp1")
class EdgeRICApp1(gym.Env):
    """EdgeRIC Env: Simulation of the realtime RIC setup + Application (Model 1)"""
    ...
    def __init__(self, config: EnvContext):
        ...
        self.observation_space = Box(
            ...
            high=np.array(
                [self.max_len_backlog, 15, self.max_len_media, self.max_len_media]
                * self.num_UEs
            ),
            dtype=np.float32,
        )
        ...
        if self.augment_state_space:
            self.observation_space = Box(
                low=np.array([0, 1, 0, 0, 0] * self.num_UEs),
                high=np.array(
                    [
    ...
    def reset(self):
        ...
        self.media_caches = [0] * self.num_UEs
        ...
        if self.augment_state_space:
            init_state = np.array(
                ...
                dtype=np.float32,
            )  # [BL1, CQI1, BP1, MB1, MC1, BL2, CQI2, BP2, MB2, MC2.....]
        else:
            init_state = np.array(
                [
                    param[ue]
                ...
                dtype=np.float32,
            )  # [BL1, CQI1, MB1, MC1, BL2, CQI2, MB2, MC2,.....]
    ...
    def step(self, action):
        ...
        2.) Backlog buffer to playback(media) cache to playback(media) buffer
        ...
        for ue in range(self.num_UEs):
            ...
            # Playout from UE
            inter_departure_time, chunk_size, GoP_size = self.playout_process_params[ue]
            self.media_caches[ue] += bytes_transferred
            completed_GoPs = np.floor(self.media_caches[ue] / GoP_size)
            self.media_caches[ue] -= completed_GoPs * GoP_size
            self.media_lens[ue] += completed_GoPs * GoP_size
            self.media_lens[ue] = min(self.max_len_media, self.media_lens[ue])
