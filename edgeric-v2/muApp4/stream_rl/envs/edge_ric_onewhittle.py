...
class EdgeRIC(gym.Env):
    ...
    def step(self, action, RNTI, CQI, BL, tx):
        ...
        for ue in range(self.num_UEs):
            ...
            if self.t % inter_arrival_time == 0:
                if self.app == "urllc" or self.app == "mmtc":
                    self.interarrivaltimebursty = self.t+int(np.random.choice([50,100,150,200],size=1,p=None))
        ...
        if action == self.action_space.n-1:
            ...
            print("cost of high: " + str(self.cost_high_action) + " reward: " + str(reward))    
        # elif action == self.action_space.n-2:
        #     reward = (
        #         self.reward_func(total_bytes_transferred, self.backlog_lens)/1000
    ...
    def get_metrics_multi(self):
        ...
        try:
            ...
            if(len(messagedata)!= (self.numArms*(numParams+2) + 3)):
                #print("I am here")
                return self.r, self.cqis[0], self.backlog_lens[0], self.tx_neurwin
