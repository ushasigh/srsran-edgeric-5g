...
def collect_samples(
    ...
    while num_steps < min_batch_size:
        ...
        for t in range(10000):
            ...
            RNTI, CQI, BL, tx = env.get_metrics_multi()
            #next_state, reward, done , _, _ = env.step(action)
            next_state, reward, done , _, _, _ = env.step(action, RNTI, CQI, BL, tx)
            reward += 0.0*reward
            # next_state, reward, tru, ter, _ = env.step(action)
            # done = tru or ter
...
class Agent:
    def __init__(
        self, env, policy, device, custom_reward=None, running_state=None, num_threads=1
    ):
        self.env = env
        self.policy = policy
        self.device = device
